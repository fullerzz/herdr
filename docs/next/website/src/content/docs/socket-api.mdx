...
These responses include a read-only `progress_bar` object when a pane reports visible OSC 9;4 progress:
...
`state` is one of `normal`, `error`, `indeterminate`, or `paused`. `progress` is `0` through `100`. Hidden progress is omitted.
